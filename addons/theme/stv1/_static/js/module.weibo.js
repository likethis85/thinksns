...
M.addEventFns({
	...
	loadTipoff:{
		click:function(){
			...
			$.post(U('widget/FeedList/getTipoffDetail'),{tipoff_id:attrs.tipoff_id},function(res){
				html = '';
				html += '<dl class="comment">';
				if(res == 0){
					html += '<dt class="arrow bgcolor_arrow"><em class="arrline">◆</em><span class="downline">◆</span>';
					...
					html += '<div><a href="javascript:void(0);" onclick="$(\'#tipoff_'+attrs.feed_id+'_'+attrs.tipoff_id+'\').slideToggle()" class="ico-pack-up mr10">收起</a><a target="_blank" href="'+res.tipoff_url+'" class="ico-show-all">查看原文</a></div>';
					html += '<div class="content clearfix weiba-detail"><h3>'+res.title+'</h3><p class="f9"><a class="date right">'+res.publish_time+'</a><span class="mr15">楼主：'+res.author+'</span><span>来自<a target="_blank" href="'+res.category_url+'">'+res.from_category+'</a></span></p><div><p>'+res.content+'</p></div></div>'
					html += '<div><a href="javascript:void(0);" onclick="$(\'#tipoff_'+attrs.feed_id+'_'+attrs.tipoff_id+'\').slideToggle();window.location.hash=\'#feed'+attrs.feed_id+'\'" class="ico-pack-up mr10">收起</a><a target="_blank" href="'+res.tipoff_url+'" class="ico-show-all">查看原文</a></div>';
				}
				html += '</div>';
				$('#loading').remove();
				$(_this).parent().parent().find('.feed_img_lists').html(html).slideToggle();
