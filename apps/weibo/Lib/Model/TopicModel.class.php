...
class TopicModel extends Model{
	...
	function addTopic( $content, $weiboId = false, $data = false ){
        ...
        //songxin add
		preg_match_all("/#([^#]*[^#^\s][^#]{1,40})#/is",$content,$arr);
		$arr = array_unique($arr[1]);
		$added = array();
		foreach($arr as $v){
			$data['symbol']=M('symbols')->where('symbol_char like "%|'.$v.'|%"')->find();
			if(!empty($data['symbol']['name'])&&!in_array($data['symbol'],$added)){
				$this->addKey($data['symbol']['name'], $weiboId, $data);
				array_push($added,$value['symbol']);
			}else{
	...
	public function deleteWeiboJoinTopic($content, $weiboId) {
		...
		foreach($arrlist as $key=>$value){
			$data['symbol']=M('symbols')->where('symbol_char like "%|'.$value.'|%"')->find();
			if(!empty($data['symbol']['name'])){
				array_push($arr[1],$data['symbol']['name']);
			}
		}
		//songxin end
		$arr = array_unique($arr[1]);
