...
//songxin add
	function loadLiveMore($uid,$lastId,$type=null,$follow_gid=null){
	    return $this->__optToLiveNew( $uid,$lastId,$type,$follow_gid,20,true );
	}
	//获取未取出来的新微博条数
	function countLiveNew($uid,$lastId,$type=null,$follow_gid=null){
	    $lastId = intval($lastId);
	    if(empty($lastId)) return 0;
        return $this->__optToLiveNew( $uid,$lastId,$type,$follow_gid );
	}
	
	function loadLiveNew($uid,$lastId,$limit,$type=null,$follow_gid=null){
	    return $this->__optToLiveNew( $uid,$lastId,$type,$follow_gid,$limit);
	}

	private function __optToLiveNew($uid,$lastId,$type='index',$follow_gid=null,$limit = 0,$since = false){
   ...
   //获取首页微博列表
    function getSymbolList($uid, $type='index', $since, $row=20, $gid='') {
    	$row = $row?$row:20;
		if($type=='original'){  //原创
