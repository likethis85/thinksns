...
class WeiboModel extends Model{
	 ...
	 */
     function publish($uid, $data, $from=0, $type=0, $type_data, $sync, $from_data){
     	$data['content'] =t( $data['content'] ,true); //songxin delete for 格式化 add true
    ...
    function doSaveWeibo($uid, $data, $from=0, $type=0, $type_data=null, $from_data=null){
		...
		$save['nodeid']		= intval($data['nodeid']); //songxin add 主站留言对应
        // 微博内容处理
        if(Addons::requireHooks('weibo_publish_content')){
			...
			//songxin 发文处理
			/*$contentstr="";
			$chars = preg_split('/#([^#]*[^#^\s][^#]{1,60})#/is', $save['content']);
			preg_match_all('/#([^#]*[^#^\s][^#]{1,60})#/is', $save['content'],$splitarr);
			foreach($chars as $key1=>$value1){
				$repalced = array();
				$list = M('symbols_filter')->order("w desc")->findAll();
				foreach($list as $key=>$value){
					$count = 0;
					if(!in_array($value['symbol'],$repalced)){
						$value1 = preg_replace("/(".$value['name'].")/is",'#'.$value['name'].'#',$value1,1,$count);	// 关键字过滤
						if($count>0)
						array_push($repalced,$value['symbol']);
					}
				}
				$contentstr.=$value1;
				if(!empty($splitarr[0][$key1])){
					$contentstr.=$splitarr[0][$key1];
				}
			}
			$save['content']=$contentstr;*/
			//songxin end
