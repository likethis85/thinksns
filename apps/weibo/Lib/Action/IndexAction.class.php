...
//songxin add
    //加载更多的
    function loadlivemore(){
        $data['showfeed'] = intval($_REQUEST['showfeed']);
        $data['lastId'] = intval($_POST['since']);
        $data['type']   = t($_POST['type']);
        $data['follow_gid'] = intval($_POST['follow_gid']);
        $uid = isset($_POST['hasUid']) ? intval($_POST['hasUid']):8;
    	$data['list'] = D('Operate')->loadLiveMore($uid,$data['lastId'],$data['type'],$data['follow_gid']);
    	$this->assign($data);
    	$this->display("loadlive");
    }
    //查看最新的
    function countlivenew(){
    	$data['showfeed'] = intval($_REQUEST['showfeed']);
    	$data['lastId'] = intval($_POST['lastId']);
    	$data['type']   = intval($_POST['type']);
    	$data['follow_gid'] = intval($_POST['follow_gid']);
    	$uid = isset($_POST['hasUid']) ? intval($_POST['hasUid']):8;
    	$data['limit'] = D('Operate')->countLiveNew($uid,$data['lastId'],$data['type'],$data['follow_gid']);
    	$this->assign($data);
    	$this->display();
    }

    function loadlivenew(){
    	$data['showfeed'] = intval($_REQUEST['showfeed']);
    	$data['lastId'] = intval($_POST['since']);
    	$data['type']   = t($_POST['type']);
    	$data['follow_gid'] = intval($_POST['follow_gid']);
    	$data['limit']      = intval($_POST['limit']);
    	$uid = isset($_POST['hasUid']) ? intval($_POST['hasUid']):8;
    	$data['list'] = D('Operate')->loadLiveNew($uid,$_POST['since'],$data['limit'],$data['type'],$data['follow_gid']);
    	$this->assign($data);
    	$this->display('loadlive');
    }
//songxin end
