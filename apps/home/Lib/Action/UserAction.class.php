...
class UserAction extends Action {
    ...
    const INDEX_TYPE_NEWS  = 3; //songxin add
    ...
    function index() {
        ...
        $index_type = intval( $_GET ['type'] );  //0=我关注的、1=群组微博、2=正在发生 3=新闻频道songxin 
    ...
    private function __assignTabSwitch ($index_type)
    {
        ...
        switch ($index_type) {
            ...
            case self::INDEX_TYPE_ALL:
                ...
                break;
            case self::INDEX_TYPE_NEWS:
                $this->assign('news_tab', 'on');
                break;
            default:
                $this->assign('weibo_tab','on');
        }
    ...
    {   
		...
		$data['symbol']=M('symbols')->where('symbol_char like "%|'.t($data['search_key']).'|%"')->find();
		if(!empty($data['symbol']['name'])&&$data['symbol']['name']!=t($data['search_key'])){
		   redirect(U('home/User/topics', array('k'=>$data['symbol']['name'],'jump'=>$data['search_key']))); //同类词跳转
		}
		//songxin add end
...
//      $data['list']['count'] = D ( 'Operate', 'weibo' )->where("content LIKE '%#{$data['topics']['name']}#%' AND isdel=0")->count();
    ...
    private function __getWeiboList($install_app,$index_type,$weibo_type)
    {
        ...
        switch ($index_type) {
            ...
            case self::INDEX_TYPE_NEWS:
                $order = 'weibo_id DESC';
                $data['list']  = D('Operate','weibo')->doSearchTopic('`uid` = 2 or `uid` = 3 or `uid` = 8', $order,$this->mid); //2:breakingnews 3:wallstreetcn 8:live
                break;
			//songxin end
            default:
                $data ['list'] = D('Operate', 'weibo')->getHomeList ( $this->mid, $weibo_type, '', '', $data ['follow_gid'] );
