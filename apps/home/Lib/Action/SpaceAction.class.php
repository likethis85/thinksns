...
class SpaceAction extends Action
{
	function _initialize() {
		if (!is_numeric($_GET['uid']) && is_string($_GET['uid'])&&'comment' != ACTION_NAME) { //songxin add
		...
		if ('detail' != ACTION_NAME && 'comment' != ACTION_NAME) { //songxin edit
			$user_info = D('User')->getUserByIdentifier($this->uid);
	   	   	if ($user_info) {
	   	   		$userinfo = array(
	   	   						...
	   	   						//'性别'    => getSex($user_info['sex']), songxin delete
    ...
    public function index()
    {
        ...
        //$data['list'] = D('Operate','weibo')->getSpaceList($this->uid, $type);
        ...
        $this->assign($data);
        $this->setTitle($data['user']['uname'] . '的个人主页');
    	$this->display();
    }
    ...
    function comment(){
		...
		if(M('weibo')->where('nodeid='.$nodeId.' and isdel=0 ')->count()==0){
		$content=t(urldecode($_GET['nodetitle']))." http://wallstreetcn.com/node/".$nodeId ;
		
		$intId=D('Weibo','weibo')->publish(3,
									array(
			...
			$intId = M('weibo')->where('nodeid='.$nodeId.' and isdel=0 ')->find();
			$intId = intval( $intId['weibo_id'] );
		}
