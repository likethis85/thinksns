...
if($_GET['t'] == 'attach') {
	...
	if(empty($attach_list)) {
		...
		foreach($attach_list as $value) {
			...
			$data[] = "('".$value['id']."',null,null,null,'".$value['attach_type']."','".$value['userId']."','".$value['uploadTime']."','".$value['name']."','".$value['type']."','".$value['size']."','".$value['extension']."','".$value['hash']."','".$value['private']."','".$value['isDel']."','".$value['savepath']."','".$value['savename']."','".$value['savedomain']."','0')";
		}
