...
function getUserSpace($uid,$class,$target,$text, $icon = true) {
   ...
   //songxin add
	$groupIds=array(2); //symbol group
	if(!model('UserGroup')->isUserInUserGroup($uid,$groupIds)){
		...
		if ($_userinfo[$uid]['domain'])
			if($uid==8 || $uid==2 || $uid==3){
				$url = "http://t.wallstreetcn.com/".$_userinfo[$uid]['domain'];
			}else{
				$url = U('home/Symbol/index', array('uid' => $_userinfo[$uid]['domain']));			
			}

			//$url = "../../$".$_userinfo[$uid]['domain'];
		else
			$url = U('home/Symbol/index', array('uid' => $uid));
		
		$user_space_info = "<a href='{$url}' class='{$class} nocard' target='{$target}'>$text" . $user_name_end . "</a>";
	}
	//songxin end
...
function getShort($str, $length = 40, $ext = '') {
	...
	//$str	=	strip_tags($str);
	$str	=	strip_tags($str,"<br><br />");//songxin edit
...
function format($content,$url=false){
	if($url){
	 ...
	 foreach ($matches[0] as $longword)
		$content = str_replace ($longword, '<a href="'.$longword.'" target="_blank">'. substr ($longword, 0, 30) . '..</a>', $content);
	...
	$content = preg_replace_callback("/#([^#][^#^\s][^#]{1,40})#/is",themeformat,$content);
	//$content = preg_replace_callback("/\\$([^\\$^\s]{0,20})\\$/is",themeDollorformat,$content); //add $解析
	$content = preg_replace_callback("/@([\w\x{2e80}-\x{9fff}\-]+)/u",getUserId,$content);
	$content = keyWordFilter($content);
    return html_entity_decode($content); //songxin
}
...
function formatComment($content,$url=false){
    ...
    $content = preg_replace_callback("/#([^#]*[^#^\s][^#]{1,40})#/is", group_themeformat, $content);
	//$content = preg_replace_callback("/\\$([^\\$][^\\$^\s][^\\$]{0,20})\\$/is",themeDollorformat,$content); //add $解析
	$content = preg_replace_callback("/@([\w\x{2e80}-\x{9fff}\-]+)/u",getUserId,$content);
	$content = keyWordFilter( $content );
	return html_entity_decode($content); //songxin
...
//songxin $解析
function themeDollorformat($data){

	//return "<a href=".U('home/Symbol/index',array('uid' => urlencode($data[1]))).">".$data[0]."</a>";
	return "<a href=".U('home/user/topics',array('k'=>urlencode($data[1]))).">".$data[0]."</a>";
