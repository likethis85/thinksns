...
function format($content,$url=false){
	...
	$content = preg_replace_callback("/#([^#][^#^\s][^#]{1,20})#/is",themeformat,$content);
	$content = preg_replace_callback("/\\$([^\\$^\s]{0,8})/is",themeDollorformat,$content); //add $解析
...
function group_weibo_format($content, $gid, $url=false){
    ...
    $content = preg_replace_callback("/(?:#[^#]*[^#^\s][^#]*#|(\[.+?\]))/is", replaceEmot, $content);
    $content = preg_replace_callback("/#([^#]*[^#^\s][^#]{1,20})#/is", group_themeformat, $content);
	$content = preg_replace_callback("/\\$([^\\$][^\\$^\s][^\\$]{0,5})\\$/is",themeDollorformat,$content); //add $解析
    $content = preg_replace_callback("/@([\w\x{2e80}-\x{9fff}\-]+)/u", getUserId, $content);
    $content = keyWordFilter($content);
    return $content;
}
